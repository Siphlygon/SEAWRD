...
@pytest.mark.tf
@pytest.mark.slow
def test_clone_model_preserves_normalisation_behaviour():
    """
    Test that DNNManager.clone_model preserves the behaviour of the normalisation layer in the cloned model.
    
    Incredibly, despite having the same weights, the cloned model's normalisation layer may not behave the same as the
    original due to the way Keras handles the internal state of normalisation layers. This test ensures that the cloned
    model behaves identically to the original when given the same input data by testing predictions.
    """
    x = np.array([
        [1.0, 100.0, 2.0, 3.0, 4.0, 5.0],
        [2.0, 200.0, 3.0, 4.0, 5.0, 6.0],
        [3.0, 300.0, 4.0, 5.0, 6.0, 7.0],
    ], dtype=np.float32)

    normaliser = keras.layers.Normalization(axis=-1, name="feature_normalizer")
    normaliser.adapt(x)
    ...
    clone = manager.clone_model()

    np.testing.assert_allclose(
        manager.model.layers[0](x).numpy(),
        clone.layers[0](x).numpy(),
